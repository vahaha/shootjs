...
async function createComponent(componentName) {
    ...
    const dir = path.join(dirComponents, paramCase(componentName))
    if (fs.existsSync(dir)) {
        throw new Error('Folder ' + paramCase(componentName) + ' already exists.')
    }
    ...
    fs.writeFileSync(
        ...
        _genRequire(componentName, ['static', 'cache']),
        'utf8'
    )
    fs.writeFileSync(
        path.join(dir, 'cache.js'),
        _genRequire(componentName, ['model']),
        'utf8'
    )
    fs.writeFileSync(
        path.join(dir, 'service.js'),
        _genRequire(componentName, ['model', 'cache']),
        'utf8'
    )
    fs.writeFileSync(
        path.join(dir, 'controller.js'),
        _genRequire(componentName, ['service']),
        'utf8'
    )
    fs.writeFileSync(
        path.join(dir, 'route.js'),
        "const Router = require('@koa/router')\n" +
        _genRequire(componentName, ['controller']),
        'utf8'
    )
    // index.js
    fs.copyFileSync(
